...
RunMetaCell <- function(obj.seurat, dataset_name, output.prefix,
  ...
  metacell::scdb_init(outpath, force_reinit = T)
  metacell::scfigs_init(outpath)
  ...
  mat <- metacell::scm_import_sce_to_mat(sce)
  metacell::scdb_add_mat(dataset_name, mat)

  metacell::mcell_plot_umis_per_cell(dataset_name)
  # ignore cells
  metacell::mcell_mat_ignore_small_cells(dataset_name, dataset_name, min_UMI)
  ...
  metacell::mcell_gset_filter_varmean(
    gstat_id = dataset_name,
  ...
  metacell::mcell_gset_filter_cov(gset_id = dataset_name, gstat_id = dataset_name, T_tot = T_tot, T_top3 = T_top3)

  if (isTRUE(filter_bad_genes)) {
    # all_genes <- c(rownames(mat@mat), rownames(mat@ignore_gmat))
    gset <- metacell::scdb_gset(dataset_name)
    ...
    metacell::mcell_mat_ignore_genes(new_mat_id = dataset_name, mat_id = dataset_name, bad_genes, reverse = F)
  }



  # plot all genes vs selected gene
  metacell::mcell_plot_gstats(gstat_id = dataset_name, gset_id = dataset_name)
  ...
  # plot outliers
  metacell::mcell_plot_outlier_heatmap(mc_id = mc_name, mat_id = dataset_name, T_lfc = T_lfc)
  # filter
  metacell::mcell_mc_split_filt(
    new_mc_id = mcf_name,
    mc_id = mc_name,
    mat_id = dataset_name,
